...
/**
 * \file    statistique/admin/setup.php
 * \ingroup statistique
 * \brief   Statistique setup page.
 */
...
require_once '../lib/statistique.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "statistique@statistique"));
...
$arrayofparameters = array(
	...
	'STATISTIQUE_KEY_PUBLIC_PAGE'=>array('type'=>'securekey','css'=>'minwidth500', 'enabled'=>1),
	//'STATISTIQUE_TAG_PLATFORME'=>array('css'=>'minwidth500', 'enabled'=>1),
	'STATISTIQUE_TAG_PLATFORME'=>array('type'=>'category:'.Categorie::TYPE_SUPPLIER, 'enabled'=>1),
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/statistique/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = GETPOST('object');
	$constforval = 'STATISTIQUE_'.strtoupper($tmpobjectkey)."_ADDON";
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_statistique@statistique');

// Configuration header
$head = statistiqueAdminPrepareHead();
dol_fiche_head($head, 'settings', '', -1, "statistique@statistique");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("StatistiqueSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'STATISTIQUE_'.strtoupper($myTmpObjectKey).'_ADDON';
